...
que tínhamos colocado no fim de algumas instruções e a enxugar o
...
\index{em tempo de execução!erro}
\index{exceção}
...
\index{formal!linguagem}
\index{natural!linguagem}
...
declaração, ou seja, a forma como os símbolos(tokens) são organizados. A expressão
...
\item[ambiguidade:] As linguagens naturais são repletas de ambiguidade, pelas quais
as pessoas contornam usando pistas contextuais e outras informações.
As linguagens formais são projetadas para serem totalmente ou quase sem ambiguidade,
o que significa que qualquer declaração tem exatamente um único significado,
independentemente do contexto.

\item[redundância:] Para compensar a ambigüidade e reduzir
desentendimentos, as linguagens naturais empregam muita
redundância. Como resultado, frequentemente elas são prolixas. As linguagens formais
...
Se eu disser: ``A ficha caiu'', provavelmente não há ficha nem
...
Pessoas que crescem falando uma linguagem natural---todos---muitas vezes passam por um
momento difícil para se acostumar às linguagens formais. De certa forma, a diferença
...
resposta emocional. A ambiguidade não é apenas comum, frequentemente
intencional.

\item[Prosa:] O significado literal das palavras é mais importante
e a estrutura contribui dão mais sentido. A prosa é mais receptível à
análise do que a poesia, mas ainda muitas vezes ambígua.

\item[Programas]: O significado de um programa de computador é exato
e literal, e pode ser inteiramente compreendido pela análise de
tokens(símbolos) e da estrutura.

\end{description}

Aqui vão algumas sugestões para a interpretação de programas (e outras linguagens
...
de cima para baixo e da esquerda para a direita. Em vez disso, aprenda a analisar o
programa em sua mente, identificando tokens e interpretando a
estrutura. Finalmente, os detalhes fazem a diferença. Pequenos erros na
ortografia e na pontuação, aos quais você pode passar despercebido
nas linguagens naturais, podem fazer uma grande diferença em uma linguagem
formal.
...
\end{verbatim}
%
Este é um exemplo de uma {\bf instrução de impressão}, a qual,
...
\index{aspas}
\index{instrução de imprimir}
\index{de imprimir!instrução}
...
em funções no Capítulo~\ref{capfunç}.
\index{função} \index{função de imprimir} \index{Python 3}
...
que, há muito poucas diferenças que temos a nos preocupar. %%rev


\section{Depuração (Debugging)}
\index{depuração(debugging)}
