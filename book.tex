...
\item Louis Cordier notou um ponto no Capítulo 16, onde o código %rev n
não correspondia ao texto.

\item Brian Cain sugeriu várias clarificações nos capítulos 2 e 3.

\item Rob Black enviou um bando de correcções, incluindo algumas %rev n
...
e outras melhorias atenciosas. %rev
...
no mesmo dia. Estranho. %rev

\item Charles Thayer nos encorajou a se livrar dos ponto-e-vírgulas %rev n
que tinhamos colocado no fim de algumas declarações e a limpar o nosso
uso de ``argumento'' e ``parâmetro''.

\item Roger Sperberg apontou uma parte distorcida de lógica no Capítulo 3. %rev n

\item Sam Bull apontou um parágrafo confuso no Capítulo 2. %rev

\item Andrew Cheung apontou duas instâncias de ``uso antes de def.'' %rov
...
\item Wim Champagne encontrou um cérebro-o em um exemplo de dicionário. %rev n

\item Douglas Wright apontou um problema com a divisão por baixo %rev n
{\tt arc}.

\item Jared Spindor encontrou alguns refugo no final de uma frase. %rev n

\item Lin Peiheng enviou uma série de sugestões muito úteis.

\item Ray Hagtvedt entregou dois erros e um erro-não-muito. %rev

\item Torsten H\"{u}bsch apontou uma inconsistência no Swampy. %rev
...
\item Adam Hobart corrigiu um problema com a divisão por baixo em {\tt arc}. %rev

\item Daryl Hammond e Sarah Zimmerman destacou que eu servi
 {\tt math.pi} muito cedo. E Zim avistou um erro de digitação. %rev
...
\item Animesh B me ajudou a limpar um exemplo confuso. %rev

\item Martin Caspersen encontrou dois erros de arredondamento. %rev
